...
冻结事件时刻为 $(0.2,0.7,1.4,2.0)$，观察窗明确延伸到 $T=2.5$，参数 $(\mu,\alpha,\beta)=(0.8,0.3,1.2)$，初始激发为 $0.1$。递推实现与独立数值积分都得到对数似然 $-2.940341$；若把观察窗缩到最后事件之前，程序直接拒绝。季节调整例的基准强度为 $4/3$，变换后等待残差均值为 1。由此，观察窗、初始历史与季节暴露均不是只写在正文里的口头约定。
