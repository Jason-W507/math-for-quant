...
\begin{itemize}
  \item 直接先修：第 \ref{ch:lower-stat-arb-model} 章。
...
\]
取均值 $0,3$、方差 $0.2$、持续概率 $0.95$，最后两期大观测令状态 1 概率接近 1。成熟库的 Markov switching 最大似然估计用于对照；透明实现只验证给定参数的 Bayes 过滤。标签交换意味着“状态 1=牛市”不是识别约束\cite{hamilton1989new}。
