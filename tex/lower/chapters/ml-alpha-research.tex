...
生产系统需要 champion--challenger 与回退规则。复杂模型失效时，回退到冻结树基线或零仓，而不是临时挑另一个最近表现好的模型。模型注册表保存版本、数据截止、审批状态、适用市场、预期延迟和停止条件。

\MFQLead{回测、纸面交易与影子验证}

历史回测检验时间协议和过去情景，纸面交易检验实时数据、延迟和信号稳定，影子模式在不下单时记录本应产生的订单和成交假设。三者证据强度递增，但都不等于实盘收益保证。

最终测试集打开后，若继续修改模型，原测试集就成为开发数据。应发布失败结果并建立新的未来验证期，而不是反复“修到测试集变好”。这也是为什么版本、实验账本和不可变数据快照必须进入研究包。

\begin{diagnostic}
若收益只来自极少数日期、单一行业或无法成交的小盘股，模型指标再漂亮也不是稳健 Alpha。逐笔归因、暴露分解和流动性过滤应在宣称改进之前完成。
\end{diagnostic}

\MFQLead{文本 Alpha 的时间协议}
