...
\MFQTransition{Attention：内容寻址而非免费记忆}
...
\input{tex/figures/generated/lower-ml-attention.tex}

多头注意力让不同子空间学习不同关系，但头数增加不保证可解释性。注意力权重描述模型内部加权，不等同于特征的因果贡献或忠实解释。

\MFQTransition{三 token 手算}
...
\]
第三个 token 即使键更大也必须被 mask 成零权重。这个简单例子应成为实现因果 mask 的单元 解析参照。

\MFQTransition{位置、时间间隔与金融日历}
...
\MFQTransition{Transformer 块与训练稳定性}
...
\input{tex/figures/generated/lower-ml-transformer.tex}

编码器适合表示一段已经完整可见的历史。自回归解码器通过因果掩码预测下一个词元；编码器--解码器结构适合条件生成。用未来完整窗口做“编码器输入”再预测窗口中间标签是典型泄漏。

\MFQTransition{从语言模型目标到 LLM}

自回归语言模型最大化
\[
\sum_{t=1}^T\log p_\theta(w_t\mid w_{<t}),
\]
masked language model 则重建被遮蔽 token。Tokenizer 把文本映射为 subword ID；更换 tokenizer 会改变序列长度、未知词和数值表示，因此已有权重需要与相应的分词规则配合，才能使输入编号保持原来的含义。

预训练获得的语言表示也包含训练语料中的知识。即使输入的是历史新闻，模型仍可能通过参数记住事件的后续发展。研究历史预测时，需要区分模型训练数据的截止时间、权重的公开时间、新闻的首次发布时间及文本修订时间；它们共同决定模拟中的信息是否确实在当时可得。

\MFQTransition{微调、指令学习与参数高效适配}

全量微调更新全部参数，成本高且易在小样本上遗忘。事先确定编码器只训练任务头是最稳基线。LoRA 将权重更新限制为低秩矩阵
\[
W'=W+BA,\qquad A\in\mathbb R^{r\times d_{\rm in}},
\quad B\in\mathbb R^{d_{\rm out}\times r},\quad r\ll d,
\]
显著减少可训练参数\cite{hu2022lora}。Adapter、prefix tuning 和 prompt tuning 是其他参数高效路径。参数少不意味着选择偏差少：秩、目标层、学习率和 prompt 模板仍需在训练/验证协议中事先确定。

指令微调和偏好对齐改变模型回答方式，不保证事实正确。在文本分类或信息提取中，明确输出类别、保留对应原文并允许无法判断的结果，有助于比较模型输出与文本内容。把模型生成的情绪标签直接当真值，会把提示词偏差和幻觉写进因子。

\MFQTransition{检索增强、工具使用与结构化抽取}

RAG 把检索文档 $D$ 作为条件：$p(y\mid x,D)$。历史研究必须使用按决策时点构建的版本化索引；今天的知识库检索 2018 年问题会泄漏后续文档。向量库还要保存 embedding 模型、切块规则、索引时间和过滤条件。

\input{tex/figures/generated/lower-ml-llm-pipeline.tex}

工具调用适合把计算交给确定性程序，例如财务比率、日期解析和单位换算。LLM 负责选择工具与组织解释，数值 解析参照 由代码给出。研究材料应保存 prompt、工具输入输出、模型版本、温度、重试和人工覆盖记录。

\MFQTransition{LLM 的评价不能只看一个准确率}
...
\subsection{记忆衰减与注意力的数值含义}
用线性递推 $h_t=ah_{t-1}+bx_t$、$h_0=0$ 说明长程依赖。展开得 $h_T=b\sum_{s=1}^Ta^{T-s}x_s$，所以 $\partial h_T/\partial x_s=ba^{T-s}$。$|a|<1$ 时远处输入的梯度衰减，$|a|>1$ 时可能爆炸。这是 RNN 难以保留长记忆的最小例子。

LSTM 将状态更新写为 $c_t=f_t\odot c_{t-1}+i_t\odot\tilde c_t$、$h_t=o_t\odot\tanh c_t$；三个门由当前输入与旧隐状态的仿射映射经 sigmoid 得到，候选状态用 tanh。若暂时固定门，沿单元状态的直接导数为 $f_t$；让 $f_t$ 接近 1 可保留信息，但完整梯度仍包含门本身的依赖，不能声称彻底消除了梯度问题。

注意力例中前两个值为 $(10,20)$，键得分为 $(0,1)$，输出是 $(10+20e)/(1+e)\approx17.31$。第三个未来值即使改成一百万，正确的因果位置输出仍不变。notebook 显示得分矩阵、掩码、逐行权重和最终输出，并对照去掉掩码的变化。对已完整可见的历史窗口，双向编码本身并不泄漏；关键是预测时窗口是否已知。
