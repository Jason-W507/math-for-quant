...
\begin{diagnostic}
先画水平、差分和滚动尺度，再冻结确定性项与最大滞后。看到一个小 p 值以后才选择含趋势或不含趋势的规格，会把规格搜索藏进检验。
\end{diagnostic}

\section{Engle--Granger 与误差修正}

若 $x_t,y_t$ 都是 $I(1)$，但存在 $\beta$ 使 $z_t=y_t-a-\beta x_t$ 为 $I(0)$，则二者协整。Engle--Granger 两步法先估长期关系，再对残差做专用单位根检验\cite{engle1987cointegration}。冻结合成数据令
\[
y_t=1.5+2x_t+0.55^t,
\]
...
\begin{itemize}
  \item 合成正确性 oracle：\path{data/fixtures/stat-arb-model.json}；
  \item 公开领域宏观快照：\path{data/real/stat-arb-us-macro-1999q4-2009q3.json}。
\end{itemize}
快照保存来源、频率、观察截止和 SHA-256。它只能证明数据适配器与估计管线可复现，不能把 GDP 与消费称作统计套利交易对。

\begin{implementationnote}
\begin{lstlisting}[language=bash]
uv run python notebooks/lower/stat_arb_model.py \
  evidence/stat-arb-model/oracle.json
\end{lstlisting}
该 notebook 包含手算 oracle、残差图、爆炸序列故障注入、参数敏感性与限制说明。
\end{implementationnote}

本单元向 Route Capstone 交付冻结的长期关系、单位根规格、ECM 参数与 OU 时间尺度；交易规则尚不在本章作出。

\MFQLead{单位根检验的原假设与检验力}
...
\]
只有 $0<\phi<1$ 时才有正的均值回归半衰期；$\phi\le0$ 表示振荡，$\phi\ge1$ 不均值回归。半衰期是模型参数，不是保证持仓在该时间盈利。
