...
\]
合成残差给出 $\hat\phi\approx0.5442$、半衰期约 $1.1393$ 个周期。$1/\hat\kappa$ 只是特征回归时间，不是首次通过时间。若先中心化令 $\mu=0$，从 $z_0>0$ 首次命中均值边界 0，扩散率为 $\sigma$，边值问题的解为
\[
\mathbb E_{z_0}\tau_0
=\frac{\sqrt\pi}{\kappa}
...
滚动 $\hat\beta_t$ 变化意味着对冲比率变化，也可能说明关系失效。若每期重新估计并立刻用新 beta 重写过去价差，历史信号会被回看修订；研究计算记录要保存当时版本的 beta 和价差。
