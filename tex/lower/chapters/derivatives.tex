...
事先确定的八步增量给出离散二次变差 $0.86$，而目标区间长度为 1。有限网格不应被说成已经“证明”极限；它只展示网格误差。本章还核对离散恒等式
\[
...
连续时间复制要求连续调仓且无成本。离散实现从期权费中买入 $\Delta_0$ 股、其余进入现金账户；建仓与到期前再平衡计入成本，到期按持仓市值与现金结算，不额外做一次 Delta 调仓。事先确定路径的无成本复制误差为 $1.476783$，成本后误差为 $1.391342$；融资后的成本拖累为 $0.085442$，名义成本现金流为 $0.084354$。一条只有初始建仓的一步路径另用手算式核对成本随现金账户增长的结果。
...
\section{本路线的综合项目：定价、校准与对冲研究材料}

Capstone 从复制到新目录的最小研究材料运行同一参数集，依次核对二次变差、GBM 解析路径、三种定价、逐点隐波反演、Greeks 和对冲台账。六类负例分别拒绝 Novikov 指数矩发散、错误风险中性漂移、无法 bracket 的隐波、曲面蝶式套利、非法差分步长和不足以形成置信区间的 Monte Carlo 样本。

路线摘要会列出解析假设、网格与路径预算、置信区间、校准约束、交易成本和不可声称的实盘结论。

\begin{implementationnote}
\begin{lstlisting}[language=bash]
uv run jupyter nbconvert --to notebook --execute --stdout --ExecutePreprocessor.timeout=60 notebooks/lower/ch14_derivatives_stochastic.ipynb
uv run jupyter nbconvert --to notebook --execute --stdout --ExecutePreprocessor.timeout=60 notebooks/lower/ch15_derivatives_numerics.ipynb
uv run jupyter nbconvert --to notebook --execute --stdout --ExecutePreprocessor.timeout=60 notebooks/lower/ch16_derivatives_hedging.ipynb
\end{lstlisting}
三个 notebook 分别覆盖随机分析、数值定价和离散对冲；旧版整路线入口已删除。
