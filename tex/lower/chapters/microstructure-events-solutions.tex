...
\begin{enumerate}[leftmargin=*]
  ...
  \item FIFO 账本的每一步先检查事件时间和价格档，再按订单序号从队首消费数量。若市价买单量为 $q$，对卖一档剩余量 $r_1$，先成交 $f_1=\min(q,r_1)$，更新 $q\leftarrow q-f_1$、$r_1\leftarrow r_1-f_1$，再进入下一档；撤单只能减少现存量，不能使数量为负。队列完全成交概率 $0.576810$ 是独立 oracle，程序应逐事件打印成交量、剩余量和队列位置。
