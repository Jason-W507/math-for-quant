...
Winsorize、标准化和行业中性化通常逐期横截面执行。若用全历史均值标准化，未来分布会进入过去。极端值处理阈值应在训练期冻结，并保留原始值、处理值和拒绝原因，以便区分数据错误与真实尾部。

\MFQLead{预测型 Fama--MacBeth 的标准误}
