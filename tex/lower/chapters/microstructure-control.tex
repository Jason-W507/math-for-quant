...
\]
若 bid 成交，$q_{t+1}=q_t+1$、现金减少 $b_t$；若 ask 成交，库存减一、现金增加 $a_t$。下一轮必须用 $q_{t+1}$ 计算报价。事先确定例从零库存开始，首轮 bid 成交后库存为 1，下一轮报价从 $99.5/100.5$ 下移为 $99.4/100.4$。双边同时下移，使买入变得不积极、卖出更容易，才形成真正反馈。
...
这个闭环把“模型预测—控制—成交—状态—下一次控制”连起来。若只先生成整天报价、再事后计算库存，便存在前视错误。实际做市还需要不确定成交强度、选择性成交、价格跳跃、库存边界、撤单延迟和自成交防护；本章只事先确定最小状态机。

\MFQTransition{控制比较需要独立 oracle}

透明 DP 用递归和缓存搜索状态；独立实现完整枚举三期切片。报价公式也由闭式实现逐项比较。双实现的目的不是追求两份相似代码，而是让索引、终端条件和符号错误难以共同通过。

真实数据轨把 SEC 五个位置比率换成明确标注的隐含执行概率，并将前三个位置的概率依次送入同一执行状态机。事先确定压力路径只成交 1 手、剩余 8 手，partial-fill IS 为 $0.2$；它证明真实输入确实驱动控制结果，而不是停留在描述统计。聚合摘要没有母单意图、到达价和逐笔未成交委托，因此这个结果仍不是现实 implementation shortfall 估计。

\begin{implementationnote}
运行 \path{notebooks/lower/ch21_microstructure_control.ipynb} 可重建执行账本、DP/枚举对照与库存报价反馈；非法完成率、不可完成日程和未知成交状态会被拒绝。
\end{implementationnote}

本单元向本路线的综合项目交付执行目标、停止协议、状态转移和库存反馈图\cite{avellaneda2008high}。

\MFQTransition{Implementation shortfall 的完整分解}
...
控制规则将在微观结构路线 Capstone 中与事件仿真、执行成本和库存压力一起复核。
