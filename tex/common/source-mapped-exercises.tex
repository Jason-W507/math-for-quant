...
\end{enumerate}}
\providecommand{\MFQMappedUpperChFiveHints}{\par\textbf{补充习题提示：}先比较全部驻点、边界与无穷远行为；Taylor 题用四阶导数在区间上的上界控制 Lagrange 余项。}
...
\end{enumerate}
\par\textbf{逐步核对：}对 GB-3-02，先求 $4x^3-8x+c=0$ 的全部实根，再用 $12x^2-8$ 判别局部极值；在紧区间 $[a,b]$ 上还要比较 $f(a),f(b)$ 与所有驻点值，参数临界值来自 $f'=f''=0$ 的联立。对 GB-3-08，四阶导数界给出 $|R_4|\le |x|^4/[4(0.8)^4]$；若要保证误差不超过 $\varepsilon$，解 $|x|\le[4\varepsilon(0.8)^4]^{1/4}$，并把区间限制写进报告，不能说“所有收益率”。\MFQInterviewFollowup{GB-3-02：定义域变成紧集时还要加哪些候选点？GB-3-08：如何用同一余项界选取收益率阈值？}}
...
\end{enumerate}
\subsection*{数值编程（补充习题）}
\begin{enumerate}[nosep,leftmargin=*]
\item \MFQInterviewMeta{数值编程}{12 分钟}{中}{LU/Cholesky 分解与失败诊断}{GB-3-19 LU decomposition and Cholesky decomposition}对比 LU 与 Cholesky 的前提和计算结构；实现带对角正性条件的 Cholesky，并用一个对称但不正定矩阵证明失败是模型证据而非“库坏了”。
\end{enumerate}}
\providecommand{\MFQMappedUpperChSixHints}{\par\textbf{补充习题提示：}利用 $Q^TQ=I$；谱题区分零方向与全部方向；$x^TB^TBx=\|Bx\|^2$；Cholesky 的根号参数必须为正。}
...
\end{enumerate}
\par\textbf{逐步核对：}GB-3-16 中残差分解来自 $Q^TQ=I$，先把 $y=QQ^Ty+(I-QQ^T)y$ 分成列空间和正交补，再解三角方程；不形成 $X^TX$ 是因为奇异值会平方。GB-3-17/18 要把零特征值解释为零风险方向而非零矩阵，并检查 $B$ 的列空间；GB-3-19 可逐步计算主元并检查分解残差；若改用收缩矩阵，要说明新的矩阵与原矩阵有何不同。\MFQInterviewFollowup{GB-3-16：列近似相关时 QR 为什么比正规方程稳定？GB-3-17 与 GB-3-18：零特征值对风险模型意味什么？GB-3-19：收缩或增加对角项怎样改变矩阵的特征值和含义？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperChSevenHints}{\par\textbf{补充习题提示：}旅客过程只追踪两个关键座位；随机游走利用反射对称；扑克牌按点数与花色分层计数；卷积看单位正方形截线长度；生日题使用对数乘积；Poisson 题区分理论性质与模拟证据；联合违约先写 Fr\'echet 界。}
...
\item \textbf{GB-4-10。}无重复概率为 $\prod_{k=0}^{n-1}(365-k)/365$，故重复概率为一减该乘积；实现时计算 $\sum_k\log(1-k/365)$ 再取指数。近似 $1-\exp[-n(n-1)/(730)]$ 忽略高阶碰撞项，$n$ 增大时可将近似值与精确乘积比较，观察忽略高阶项带来的误差。
...
\end{enumerate}
\par\textbf{逐步核对：}GB-4-03 的最小状态只保留“第一人座位”和“最后一人座位”是否仍空；每次被迫随机选座后按这两个状态递推，得到对称的 $1/2$。GB-4-01 写吸收差分方程并用边界解，GB-4-05 用“点数选择—花色选择—第五张”三层计数，GB-4-37 用单位正方形截线长度得到三角密度。数值题把精确式作为解析参照，模拟只核对均值/方差和置信误差；GB-4-39 先检查 $q$ 的 Fr\'echet 界，再讨论相关系数或共同因子，不能跳过可行性。\MFQInterviewFollowup{GB-4-01：有偏硬币时命中概率如何变化？GB-4-03：首位旅客按非均匀分布选座时结论如何改？GB-4-05：允许 joker 后计数如何重做？GB-4-10：闰日和非均匀出生率会破坏哪一步？GB-4-29：到达分布非均匀时如何积分？GB-4-31：非齐次强度怎样改变增量分布？GB-4-37：怎样用特征函数复核卷积？GB-4-38：非等概标签的瓶颈是什么？GB-4-39：怎样用单因子模型产生尾部相关？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperChEightHints}{\par\textbf{补充习题提示：}无偏提纯只接受 HT/TH；有限样本不能证明点假设；A 题先写清条件事件；路段车辆数利用到达率乘暴露时间；Monty Hall 与轮盘题都要把主持或机械协议写进似然。}
...
\end{enumerate}
\par\textbf{逐步核对：}GB-4-16 的接受事件 HT/TH 概率相等，先证无偏，再用几何等待求期望；GB-4-15 的有限样本只能给似然、区间或预先声明的检验，不能用“直到看起来公平”为停止规则。GB-4-25 必须区分“至少一张 A”和“指定第一张 A”的条件样本空间；GB-4-28 用 Little/暴露时间思想得到 $\mathrm{Poisson}(\lambda L/v)$，速度依赖拥堵时停留时间与到达相关，需扩充状态。\MFQInterviewFollowup{GB-4-15：序贯检验如何控制可选停止？GB-4-16：偏置 $p$ 随时间漂移会破坏哪一步？GB-4-20：主持人策略未知时需要估计什么似然？GB-4-24：三发相邻时结论如何变化？GB-4-25：若抽样信息来自“随机告诉你一张 A”会怎样？GB-4-28：随机速度如何改变占用分布？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperChNineHints}{\par\textbf{补充习题提示：}$P(M\le x)=x^n$；正态矩把 $z\phi(z)$ 写成 $-\phi'(z)$；相关取等要求中心化变量几乎处处线性相关。}
...
\end{enumerate}
\par\textbf{逐步核对：}GB-4-40 先由 $P(M\le x)=x^n$ 和 $P(m>x)=(1-x)^n$ 得分布，再积分求期望；非独立样本时这两个乘积不成立。GB-4-32 用 $\phi'(z)=-z\phi(z)$ 分部积分，并用奇函数对称性处理奇矩。GB-4-41 的等号条件是中心化变量 a.e. 线性相关；若 $g$ 被限制为指标函数，要在允许阈值集合上直接比较协方差/方差，而不是把 $[-1,1]$ 当可达值。\MFQInterviewFollowup{GB-4-32：如何由矩母函数复核偶数矩？GB-4-40：样本有厚尾时，极值渐近要改用哪类条件？GB-4-41：限制 $g$ 为指标函数后如何求最优阈值？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperChElevenHints}{\par\textbf{补充习题提示：}模式状态取仍可延伸目标的最长后缀；破产概率解二阶差分；吸收时间右端多一个常数项；排队先核验 Markov/无记忆假设；系列赛以当前胜场为状态。}
...
\end{enumerate}
\par\textbf{逐步核对：}GB-5-03 用最长可延伸后缀构造自动机，再写线性方程 $E=\mathbf1+PE$；GB-5-01 的非公平解由特征根 $1,(1-p)/p$ 给出，公平情形取极限。GB-5-05 的右端常数 1 代表先走一步，模拟设最大步数并把未吸收路径计为 censored；GB-5-07 只有在指数服务和 Poisson 到达下才是出生--死亡 Markov 链。GB-5-11 从边界反向填表，动态胜率必须把时间/比分加入状态。\MFQInterviewFollowup{GB-5-01：加入每步不同赌注后状态如何改变？GB-5-03：如何由前缀自动机直接生成转移矩阵并比较模式竞赛胜率？GB-5-05：如何避免无限循环模拟拖垮测试？GB-5-07：怎样从数据检验指数服务假设？GB-5-11：七局赛改成动态胜率时如何后向计算？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperThirteenHints}{\par\textbf{补充习题提示：}把两个等式约束矩阵记为 $A^Tw=b$，联立 $2\Sigma w+A\lambda=0$。}
...
\end{enumerate}
\par\textbf{逐步核对：}先把 $A=[\mathbf1,\mu]$、$b=(1,r_0)^T$ 写清，再由拉格朗日函数 $w^T\Sigma w+\lambda^T(A^Tw-b)$ 得 $2\Sigma w+A\lambda=0$ 和 $A^Tw=b$。求解后还需检查 $A$ 的列秩、$\Sigma$ 在可行方向上的正定性和约束残差；加入不等式后要增加原始/对偶可行、互补松弛和活跃集条件，求解器返回不等于经济约束识别。\MFQInterviewFollowup{GB-3-10：加入不等式限制后哪些 KKT 条件需要新增？}}

\providecommand{\MFQMappedUpperFourteenQuestions}{
\subsection*{数值编程与研究判断（补充习题）}
\begin{enumerate}[nosep,leftmargin=*]
...
\end{enumerate}}
\providecommand{\MFQMappedUpperFourteenHints}{\par\textbf{补充习题提示：}Newton 需同时检查残差与步长；中心差分把两侧 Taylor 展开相减；Monte Carlo 先用矩母函数给解析参照，再比较配对样本的方差。}
...
\end{enumerate}
\par\textbf{逐步核对：}Newton 每步同时记录 $x_k,f(x_k),f'(x_k),\Delta x_k$，以残差、相对步长和最大迭代共同停止；阻尼/信赖域限制远跳。中心差分的总误差可写 $C_1h^2+C_2\epsilon/h$，平衡两项得到中间最优尺度。Monte Carlo 先用 $e^{1/2}$ 和 $e^2-e$ 做解析参照，再用相同总随机预算比较 antithetic 配对样本方差和区间，不能按单次误差挑优。\MFQInterviewFollowup{GB-3-09：如何用阻尼或信赖域避免跳远？GB-7-14：控制变量与重要性抽样分别需要什么额外结构？GB-7-15：如何从截断与舍入两项估计最佳 $h$？}}
...
\end{enumerate}}
\providecommand{\MFQMappedDerivativesStochasticHints}{\par\textbf{补充习题提示：}用反射原理把越界后终点低于 $a$ 的路径映到终点高于 $a$。}
\providecommand{\MFQMappedDerivativesStochasticSolutions}{\section*{补充习题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-5-15。}令 $\tau_a=\inf\{t:W_t\ge a\}$。把在 $T$ 前到过 $a$ 但终点低于 $a$ 的路径在首次到达点之后关于水平线 $a$ 反射，它们一一对应于终点高于 $a$ 的路径，故 $P(\tau_a\le T)=2P(W_T\ge a)=2[1-\Phi(a/\sqrt T)]$。若改用可选停止证明，先对 $\tau_a\wedge m$ 截断，再验证一致可积或有界条件，最后才可令 $m\to\infty$；直接把无界停止代入鞅等式没有依据。\end{enumerate}\par\textbf{逐步核对：}数值题应同时列 $a,T$、正态尾概率和反射后的双倍关系；若边界变成 $a(t)$，反射后的路径通常不再保持同一终点分布，需改用 PDE 或边界穿越方法。\MFQInterviewFollowup{GB-5-15：若边界改为随时间变化的 $a(t)$，反射证明的哪一步不再直接成立？}}
...
\end{enumerate}}
\providecommand{\MFQMappedDerivativesNumericsHints}{\par\textbf{补充习题提示：}比较 $C+Ke^{-rT}$ 与 $P+S_0e^{-qT}$ 的终值。}
\providecommand{\MFQMappedDerivativesNumericsSolutions}{\section*{补充习题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-6-02。}考虑相同到期日 $T$、执行价 $K$ 的欧式看涨与看跌期权，利率 $r$ 和连续股息率 $q$ 为已知常数。组合 A 持有一份看涨和到期支付 $K$ 的债券，到期价值为 $(S_T-K)_++K=\max(S_T,K)$；组合 B 持有一份看跌和到期交付一股股票的预付远期，到期价值为 $(K-S_T)_++S_T=\max(S_T,K)$。预付远期的当前价值为 $S_0e^{-qT}$，因此无摩擦无套利下 $C+Ke^{-rT}=P+S_0e^{-qT}$，即 $C-P=S_0e^{-qT}-Ke^{-rT}$。若组合 A 更贵，就卖出 A、买入 B；反向价差则反向交易。实际交易还涉及买卖价差、融资、借券及结算条件。\end{enumerate}\par\textbf{逐步核对：}分别在 $S_T\ge K$ 与 $S_T<K$ 时计算两个组合的价值，再比较当前成本。美式期权的提前行权权利或随机股息会改变复制关系。\MFQInterviewFollowup{GB-6-02：美式期权或随机股息下，复制组合的哪个等式需要重写？}}
...
\end{enumerate}}
\providecommand{\MFQMappedPortfolioOptimizationHints}{\par\textbf{补充习题提示：}先解 $\min w^T\Sigma w$、$\mathbf1^Tw=1$；含均值时观察 $\Sigma^{-1}\hat\mu$。}
\providecommand{\MFQMappedPortfolioOptimizationSolutions}{\section*{补充习题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-6-14。}拉格朗日函数为 $L(w,\lambda)=w^T\Sigma w+\lambda(\mathbf1^Tw-1)$。一阶条件 $2\Sigma w+\lambda\mathbf1=0$ 给 $w=-(\lambda/2)\Sigma^{-1}\mathbf1$；代入预算约束得 $\lambda=-2/(\mathbf1^T\Sigma^{-1}\mathbf1)$，于是 $w^*=\Sigma^{-1}\mathbf1/(\mathbf1^T\Sigma^{-1}\mathbf1)$。含预期收益的解依赖 $\Sigma^{-1}\hat\mu$，小特征方向和噪声均值会被放大；可用协方差收缩、均值不确定集、权重/换手约束或 Bayesian shrinkage，并必须重新比较样本外风险和成本。\end{enumerate}\par\textbf{逐步核对：}禁止卖空后可行域有不等式，闭式解只是一项候选；应枚举/求解活跃集并检查非负性、互补松弛和预算残差，而不是把负权重截成零。\MFQInterviewFollowup{GB-6-14：禁止卖空后闭式解为何失效，应怎样识别活跃约束？}}
...
\end{enumerate}}
\providecommand{\MFQMappedPortfolioTailHints}{\par\textbf{补充习题提示：}让两个头寸各自以小概率损失、且损失事件互斥；单项 VaR 可为零而组合 VaR 为正。}
\providecommand{\MFQMappedPortfolioTailSolutions}{\section*{补充习题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-6-15。}历史法取经验损失分位数，参数法在分布假设下求分位数，ES 是最坏给定概率质量的平均；有原子时需对分位点质量部分加权。令两个头寸各以 $4\%$ 概率损失 1、事件互斥，其余概率损失 0；在 $95\%$ 水平各自 VaR 为 0，而组合有 $8\%$ 概率损失 1，VaR 为 1，故 $\mathrm{VaR}(X+Y)>\mathrm{VaR}(X)+\mathrm{VaR}(Y)$。限额系统不能把分散收益当作由 VaR 自动保证。\end{enumerate}\par\textbf{逐步核对：}先画离散损失分布并数清置信水平两侧的概率质量，再分别计算单项和组合分位点；把置信度提到 99\% 后尾部观察数更少，必须给样本量/有效尾部数和区间，并解释少量尾部数据带来的局限。\MFQInterviewFollowup{GB-6-15：把置信度提到 $99\%$ 时，需要多少尾部观测才能支撑可辩护的 ES？}}
