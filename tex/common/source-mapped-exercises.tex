...
\end{enumerate}
\par\textbf{逐步核对：}GB-3-16 中残差分解来自 $Q^TQ=I$，先把 $y=QQ^Ty+(I-QQ^T)y$ 分成列空间和正交补，再解三角方程；不形成 $X^TX$ 是因为奇异值会平方。GB-3-17/18 要把零特征值解释为零风险方向而非零矩阵，并检查 $B$ 的列空间；GB-3-19 的对角正性条件、主元和残差必须逐项记录，收缩只在有明确 PSD 目标和敏感性证据时使用。\MFQInterviewFollowup{GB-3-16：列近似相关时 QR 为什么比正规方程稳定？GB-3-17 与 GB-3-18：零特征值对风险模型意味什么？GB-3-19：何时允许收缩而不是盲目加 jitter？}}
...
\end{enumerate}}
\providecommand{\MFQMappedUpperFourteenHints}{\par\textbf{来源映射题提示：}Newton 需同时检查残差与步长；中心差分把两侧 Taylor 展开相减；Monte Carlo 先用矩母函数给解析参照，再比较配对样本的方差。}
...
\providecommand{\MFQMappedDerivativesStochasticSolutions}{\section*{来源映射替换题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-5-15。}令 $\tau_a=\inf\{t:W_t\ge a\}$。把在 $T$ 前到过 $a$ 但终点低于 $a$ 的路径在首次到达点之后关于水平线 $a$ 反射，它们一一对应于终点高于 $a$ 的路径，故 $P(\tau_a\le T)=2P(W_T\ge a)=2[1-\Phi(a/\sqrt T)]$。若改用可选停止证明，先对 $\tau_a\wedge m$ 截断，再验证一致可积或有界条件，最后才可令 $m\to\infty$；直接把无界停止代入鞅等式没有依据。\end{enumerate}\par\textbf{逐步核对：}数值题应同时列 $a,T$、正态尾概率和反射后的双倍关系；若边界变成 $a(t)$，反射后的路径通常不再保持同一终点分布，需改用 PDE 或边界穿越方法。\MFQInterviewFollowup{GB-5-15：若边界改为随时间变化的 $a(t)$，反射证明的哪一步不再直接成立？}}
...
\providecommand{\MFQMappedDerivativesNumericsSolutions}{\section*{来源映射替换题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-6-02。}构造组合 A：买一股并融资 $Ke^{-rT}$，到期价值 $S_T+K$；组合 B：买看涨、卖看跌并持有股息调整，终值为 $\max(S_T-K,0)-\max(K-S_T,0)+K=S_T$。连续股息使初始股票成本为 $S_0e^{-qT}$，所以 $C-P=S_0e^{-qT}-Ke^{-rT}$。若左侧过高则卖出昂贵组合、买入便宜组合；真实可执行性还受 bid/ask、融资、股息预测、借券、行权与结算时点约束。\end{enumerate}\par\textbf{逐步核对：}套利报告必须按两种终值逐状态比较，并把交易方向、现金流、保证金和成本列成表；美式期权或随机股息会改变提前行权/股息现金流，原复制等式不能直接沿用。\MFQInterviewFollowup{GB-6-02：美式期权或随机股息下，复制组合的哪个等式需要重写？}}
