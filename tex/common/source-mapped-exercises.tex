...
\end{enumerate}
\par\textbf{逐步核对：}GB-3-16 中残差分解来自 $Q^TQ=I$，先把 $y=QQ^Ty+(I-QQ^T)y$ 分成列空间和正交补，再解三角方程；不形成 $X^TX$ 是因为奇异值会平方。GB-3-17/18 要把零特征值解释为零风险方向而非零矩阵，并检查 $B$ 的列空间；GB-3-19 的对角门禁、主元和残差必须逐项记录，收缩只在有明确 PSD 目标和敏感性证据时使用。\MFQInterviewFollowup{GB-3-16：列近似相关时 QR 为什么比正规方程稳定？GB-3-17 与 GB-3-18：零特征值对风险模型意味什么？GB-3-19：何时允许收缩而不是盲目加 jitter？}}
...
\end{enumerate}
\par\textbf{逐步核对：}Newton 每步同时记录 $x_k,f(x_k),f'(x_k),\Delta x_k$，以残差、相对步长和最大迭代共同停止；阻尼/信赖域限制远跳。中心差分的总误差可写 $C_1h^2+C_2\epsilon/h$，平衡两项得到中间最优尺度。Monte Carlo 先用 $e^{1/2}$ 和 $e^2-e$ 做独立 oracle，再用相同总随机预算比较 antithetic 配对样本方差和区间，不能按单次误差挑优。\MFQInterviewFollowup{GB-3-09：如何用阻尼或信赖域避免跳远？GB-7-14：控制变量与重要性抽样分别需要什么额外结构？GB-7-15：如何从截断与舍入两项估计最佳 $h$？}}
...
\providecommand{\MFQMappedPortfolioTailSolutions}{\section*{来源映射替换题}\begin{enumerate}[leftmargin=*]\item \textbf{GB-6-15。}历史法取经验损失分位数，参数法在分布假设下求分位数，ES 是越过分位点的尾部平均。令两个头寸各以 $4\%$ 概率损失 1、事件互斥，其余概率损失 0；在 $95\%$ 水平各自 VaR 为 0，而组合有 $8\%$ 概率损失 1，VaR 为 1，故 $\mathrm{VaR}(X+Y)>\mathrm{VaR}(X)+\mathrm{VaR}(Y)$。限额系统不能把分散收益当作由 VaR 自动保证。\end{enumerate}\par\textbf{逐步核对：}先画离散损失分布并数清置信水平两侧的概率质量，再分别计算单项和组合分位点；把置信度提到 99\% 后尾部观察数更少，必须给样本量/有效尾部数和区间，否则只能返回 warn/reject。\MFQInterviewFollowup{GB-6-15：把置信度提到 $99\%$ 时，需要多少尾部观测才能支撑可辩护的 ES？}}
