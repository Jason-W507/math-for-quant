...
\end{lstlisting}
它同时核对解析记录、固定种子模拟、规范输入、版本和容差；篡改期望值与放宽门槛会以非零状态失败。
...
估计量需要损失与抽样记录，检验需要预先声明的错误率与停止规则，回归需要把识别和协方差估计分开，多重搜索需要独立确认。上册 Capstone 将要求每项显著性声明可追溯到时点数据、检验族、依赖结构、效应大小、成本和明确限制。
