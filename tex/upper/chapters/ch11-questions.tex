\section*{章末练习}
\begingroup\small
\subsection*{口述概念（3 题）}
\begin{enumerate}[nosep,leftmargin=*]
  \item 区分有限维分布、版本、不可区分和连续修改；为何边缘分布相同不够？
  \item 区分适应、可预测与停止时刻，并给出一个未来函数的交易例子。
  \item 沟通类、常返/暂留、稳态、极限分布、周期与详细平衡之间是什么关系？
\end{enumerate}
\subsection*{笔试推导（3 题）}
\begin{enumerate}[nosep,leftmargin=*,start=4]
  \item 解公平随机游走在 $\{0,\ldots,N\}$ 上先到 $N$ 的概率和吸收期望时长。
  \item 证明 $S_n^2-n$ 与 $N_t-\lambda t$ 是鞅，写出可预测二次变差，并解释加倍下注为何不能推出稳赚。
  \item 用反射原理推导 $\mathbb P(\sup_{s\le t}W_s\ge a)$，再说明离散监控为何低估越界概率。
\end{enumerate}
\subsection*{数值编程（3 题）}
\begin{enumerate}[nosep,leftmargin=*,start=7]
  \item 对两状态链核对 $P^5$ 与稳态，并构造周期链展示“有稳态但不收敛”。
  \item 独立核对 Poisson 的等待、叠加、稀疏化和非齐次累计强度记录。
  \item 核对 Brownian 缩放、Donsker 终值矩、协方差、二次变差与绝对增量和；写出解析基准和边界。
\end{enumerate}
\subsection*{研究判断（3 题）}
\begin{enumerate}[nosep,leftmargin=*,start=10]
  \item 订单到达呈日内季节性和聚集性时，如何逐级检验并替换齐次 Poisson？
  \item 某收益状态模型训练集拟合良好，却在制度切换后失效。如何判断是转移概率漂移还是状态定义不充分？
  \item 交易者把“全样本最大收益时刻”当作停止时刻并声称鞅期望不变。指出信息集问题，给出一个有界停止或截断修复。
\end{enumerate}
\endgroup
% MFQ source replacements removed=1 unit=UpperChEleven

\input{tex/common/source-mapped-exercises}
\MFQMappedUpperChElevenQuestions

\subsection*{基础衔接：独立计算}
公平游走在 $0,1,\ldots,5$ 上从 1 出发，求先到 5 的概率与退出期望步数。
