\chapter{线性代数、分解、投影与条件数}
\label{ch:linear-algebra}

\begin{introduction}
  \input{tex/generated/prerequisites/upper-ch06.tex}
  \item 学习产出：能从几何与优化两条路径推导最小二乘，按假设选择分解，并用前向/后向误差解释病态性。
  \item 研究连接：回归、因子风险模型、PCA、协方差估计、组合优化和数值稳定性都依赖本章的不变量。
\end{introduction}

矩阵首先是\textbf{线性映射}，其次才是二维数据容器。Python/NumPy 允许一句 \texttt{solve} 得到数字；研究者还必须回答：解是否存在、是否唯一、哪个方向不可辨识、输入误差会被放大多少，以及库函数究竟使用了什么结构。

\section{从向量空间到四个基本子空间}

向量空间对加法与数乘封闭。向量组 $v_1,\ldots,v_k$ 的张成空间是所有线性组合；若
\[
  a_1v_1+\cdots+a_kv_k=0
\]
只允许零系数，它们线性无关。一组既线性无关又张成空间的向量称为\textbf{基}，基向量个数称为维数。坐标依赖所选基，向量本身不依赖。

矩阵 $A\in\mathbb R^{m\times n}$ 表示映射 $x\mapsto Ax$。它带来四个基本子空间：列空间 $\mathcal C(A)\subseteq\mathbb R^m$、零空间 $\mathcal N(A)\subseteq\mathbb R^n$、行空间 $\mathcal C(A^T)$ 与左零空间 $\mathcal N(A^T)$。秩--零化度定理给出
\[
  \operatorname{rank}(A)+\dim\mathcal N(A)=n.
\]
并且 $\mathcal C(A)^\perp=\mathcal N(A^T)$、$\mathcal C(A^T)^\perp=\mathcal N(A)$。因此，“数据无法解释的残差”与“参数不可辨识方向”分别落在不同空间，不能笼统归为“矩阵有问题”。

\begin{example}[重复因子暴露]
若设计矩阵两列完全相同，则 $(1,-1)^T\in\mathcal N(A)$。预测 $A\beta$ 可能唯一，但系数 $\beta$ 不唯一：沿 $(1,-1)^T$ 移动不会改变拟合值。删除变量、加约束或正则化会选择一个解，却都改变了问题的解释。
\end{example}

\section{内积、正交补与投影定理}

欧氏内积为 $\langle u,v\rangle=u^Tv$，范数为 $\lVert u\rVert_2=\sqrt{u^Tu}$。若内积为零则正交。有限维子空间 $S$ 的\textbf{正交投影定理}说：对每个 $y$，存在唯一分解
\[
  y=s+r,\qquad s\in S,\quad r\in S^\perp.
\]
证明不需要先猜公式。取使 $\lVert y-s\rVert$ 最小的 $s$。若存在 $z\in S$ 使 $z^T(y-s)\ne0$，沿 $z$ 方向微调便能继续减小距离，矛盾；故残差正交。若有两个分解，相减后同一向量同时属于 $S$ 与 $S^\perp$，只能为零，故唯一。

\input{tex/figures/generated/upper-ch06-projection-svd.tex}
% Short alias: \input{tex/upper/figures/projection-svd.tex}



令满列秩 $X\in\mathbb R^{n\times d}$，投影 $s=X\hat\beta$。残差 $r=y-X\hat\beta$ 与每一列正交，因而
\[
  X^Tr=0
  \Longleftrightarrow X^TX\hat\beta=X^Ty,
  \qquad
  P_X=X(X^TX)^{-1}X^T.
\]
$P_X$ 对称且幂等：$P_X^T=P_X$、$P_X^2=P_X$；它在列空间上是恒等映射，在正交补上为零。公式中的逆矩阵要求满列秩。秩亏时投影仍唯一，但系数未必唯一，应使用正交基或 Moore--Penrose 伪逆 $X^+$，写成 $P_X=XX^+$。


\section{最小二乘：同一结论的优化证明}

考虑
\[
  L(\beta)=\frac12\lVert y-X\beta\rVert_2^2.
\]
第 5 章给出 $\nabla L=X^T(X\beta-y)$。一阶条件正是正规方程；Hessian 为 $X^TX$。满列秩时它正定，极小点唯一；秩亏时它半正定，所有极小点给出同一个投影，却可能给出不同参数。

固定算例取
\[
X=\begin{pmatrix}1&0\\1&1\\1&2\end{pmatrix},\qquad
y=\begin{pmatrix}1\\2\\2\end{pmatrix}.
\]
手算 $X^TX=\left(\begin{smallmatrix}3&3\\3&5\end{smallmatrix}\right)$、$X^Ty=(5,6)^T$，得到
\[
 \hat\beta=(7/6,1/2)^T,\quad
 r=(-1/6,1/3,-1/6)^T,
 \quad X^Tr=0,\quad r^Tr=1/6.
\]
这些数是独立基准，而不是从 NumPy 输出抄回的“期望”。

\section{分解不是菜单：先看假设，再看任务}

\subsection{QR：最小二乘的默认几何工具}

满列秩 $X=QR$，其中 $Q^TQ=I$、$R$ 上三角。因为正交变换保持二范数，
\[
 \lVert y-X\beta\rVert_2^2
 =\lVert Q^Ty-R\beta\rVert_2^2+\lVert (I-QQ^T)y\rVert_2^2.
\]
第二项与 $\beta$ 无关，只需回代 $R\beta=Q^Ty$。正规方程把条件数平方：
\[
 \kappa_2(X^TX)=\kappa_2(X)^2,
\]
所以“显式求 $(X^TX)^{-1}$”既多做工作，又会放大病态性。实际回归优先调用最小二乘/QR/SVD 求解器，而不是 \texttt{inv(X.T @ X) @ X.T @ y}。

\subsection{Cholesky 与谱分解：利用对称正定结构}

若 $A$ 对称正定，则存在唯一对角元为正的下三角矩阵 $L$，使 $A=LL^T$。Cholesky 适合重复求解协方差或 Hessian 系统，但半正定、近奇异或非对称输入会破坏其前提。

实对称矩阵满足谱定理：$A=Q\Lambda Q^T$，$Q$ 正交。协方差矩阵的特征值非负；特征向量给出互相正交的风险方向。固定矩阵
\[
  \Sigma=\begin{pmatrix}2&1\\1&2\end{pmatrix}
\]
的特征值为 $1,3$，最大方差方向与 $(1,1)^T$ 平行。若样本协方差出现小负特征值，先区分浮点误差、缺失值处理和模型错误，再决定是否修正；不能机械截断后宣称原模型有效。

\subsection{SVD：秩、伪逆与最佳低秩近似}

任意 $X\in\mathbb R^{m\times n}$ 都有
\[
 X=U\Sigma V^T=\sum_{i=1}^{r}\sigma_i u_iv_i^T,
 \qquad \sigma_1\ge\cdots\ge\sigma_r>0.
\]
右奇异向量是参数方向，左奇异向量是输出方向；$Xv_i=\sigma_i u_i$。零奇异值给出不可辨识方向，小奇异值给出高敏感方向。伪逆
\[
 X^+=V\Sigma^+U^T
\]
在所有最小二乘解中选择最小二范数解。Eckart--Young--Mirsky 定理说明
截断到前 $k$ 项的 $A_k$ 同时是谱范数与 Frobenius 范数下的最佳秩
$k$ 近似，并且
\[
 \lVert A-A_k\rVert_2=\sigma_{k+1},\qquad
 \lVert A-A_k\rVert_F=\left(\sum_{i>k}\sigma_i^2\right)^{1/2}.
\]
写“平方 Frobenius 范数”虽然有同一极小化解，却掩盖了标准误差公式。
截断 SVD 和 ridge 都降低方差，却以偏差为代价，不是免费“修复”。

固定设计矩阵的奇异值为 $2.676243$ 与 $0.915272$。最佳秩一近似的二范数误差正好是 $0.915272$；这同时核验了奇异值排序和低秩近似定理。

\section{PCA、白化与风险模型边界}

把 $n$ 个中心化观测按行放入 $Z$，样本协方差为
\[
  S=\frac{1}{n-1}Z^TZ=Q\Lambda Q^T.
\]
PCA 方向是 $Q$ 的列，得分是 $ZQ$，第 $j$ 个方向的样本方差为 $\lambda_j$。它寻找\textbf{无监督方差最大}的正交方向，不保证该方向能预测收益，也不保证在另一个样本窗口稳定。解释方差比例高，只说明压缩了当期横截面或时间序列波动，不能直接充当 alpha 证据。

若 $S$ 正定，白化变换可写成
\[
  z=\Lambda^{-1/2}Q^T(x-\bar x),
  \qquad \operatorname{Cov}(z)=I.
\]
但 $\lambda_j$ 很小时，$\lambda_j^{-1/2}$ 会放大估计噪声。截断小特征值、加 ridge 或改用收缩协方差都能抑制放大，却分别改变保留子空间、距离度量或估计目标。研究报告必须保存中心化规则、样本分母、特征值阈值、符号/排序约定和样本外稳定性；否则同一段 PCA 代码很难复现，更无法解释风险暴露为何变化。

\section{条件数、前向误差与后向误差}

对可逆矩阵，
\[
  \kappa_2(A)=\frac{\sigma_{\max}(A)}{\sigma_{\min}(A)}.
\]
它描述问题在最坏方向上的相对扰动放大，不是每次观测误差的精确倍数。若只扰动右端，在线性近似和适当非零条件下有
\[
 \frac{\lVert\delta x\rVert}{\lVert x\rVert}
 \lesssim \kappa_2(A)
 \frac{\lVert\delta b\rVert}{\lVert b\rVert}.
\]
\textbf{前向误差}比较计算解与真解；\textbf{后向误差}问计算解究竟精确解了一个多近的输入问题。稳定算法可以对病态问题产生小后向误差但大前向误差，因此“算法稳定”和“问题适定”必须分开。

取
\[
A_\varepsilon=\begin{pmatrix}1&1\\1&1+\varepsilon\end{pmatrix},
\qquad \varepsilon=10^{-6}.
\]
其 $\kappa_2\approx4.0\times10^6$。原记录把第一项加 $10^{-8}$，得到绝对分量放大 $1000001$，但相对范数放大只有约 $1.0000005$：扰动并未对准最坏方向。独立基准另用最大左奇异方向构造 $b$、最小左奇异方向构造 $\delta b$，此时相对放大约 $4.0\times10^6$，才真正逼近条件数界。

\begin{note}
列缩放可能显著改善数值条件，却改变系数单位；ridge 把 $X^TX$ 改为 $X^TX+\lambda I$，同时改变估计目标；删因子改变模型空间。三者都可能让数字更稳定，但研究含义不同，必须单独报告。
\end{note}

\section{独立计算证据与反例}

运行：
\begin{lstlisting}[language=bash]
uv run jupyter nbconvert --to notebook --execute --ExecutePreprocessor.allow_error_names=SystemExit notebooks/upper/ch06_linear_algebra.ipynb
uv run jupyter nbconvert --to notebook --execute --stdout \
  --ExecutePreprocessor.timeout=60 \
  --ExecutePreprocessor.allow_error_names=SystemExit \
  notebooks/upper/ch06_linear_algebra.ipynb
\end{lstlisting}
程序核验正规方程记录、残差正交、投影对称/幂等、QR 与 SVD 重构、伪逆解、Cholesky、最佳秩一误差、条件数平方和两种扰动方向。它还固定输入、维数与容差；把设计矩阵写入 \texttt{NaN}、改成另一组数据、放宽误差门或伪造条件数标签都会以稳定诊断失败。

\begin{remark}[Python 边界]
NumPy 的 \texttt{lstsq}、\texttt{qr}、\texttt{eigh}、\texttt{svd} 和 \texttt{cond} 是计算工具，不自动验证假设。\texttt{eigh} 只用于对称/Hermitian 问题；\texttt{eig} 的一般结果不保证实数或正交；\texttt{matrix\_rank} 的结论依赖容差。把 dtype、形状、缩放、阈值和独立记录一起保存，结果才可审计。
\end{remark}

\noindent\textbf{延伸阅读。}分解、最小二乘与数值线性代数的标准来源参见\cite{golub2013matrix}。

\input{tex/upper/chapters/ch06-questions}
\input{tex/upper/chapters/ch06-hints}

\noindent\textbf{本章小结与 Capstone 连接}\quad
先用子空间说明可解释与不可辨识方向，再用分解匹配结构和任务，最后用条件数、前后向误差与反例界定可信范围。上册 Capstone 将同时审计残差、重构、奇异值谱、扰动方向和正则化后的研究含义。
