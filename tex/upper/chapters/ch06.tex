...
\[
 ...
 =\begin{pmatrix}b_0\\b_0+b_1\\b_0+2b_1\end{pmatrix}.
\]
两列 $v_1=(1,1,1)^T$、$v_2=(0,1,2)^T$ 的组合包含所有可由直线产生的预测。若 $b_0v_1+b_1v_2=0$，第一行给 $b_0=0$，第二行再给 $b_1=0$；所以两列独立，构成列空间的一组基，列空间维数为 2。

任意列空间向量 $s$ 都满足 $s_1-2s_2+s_3=0$。反过来，满足该等式时，取 $b_0=s_1,b_1=s_2-s_1$ 即可恢复 $s=Xb$。因此列空间是观测空间 $\mathbb R^3$ 中的一个平面。观测 $y=(1,2,2)^T$ 不满足这个等式，因为 $1-4+2=-1$；这就解释了为什么三点不能落在同一条直线上。

零空间解决另一个问题：哪些参数变化不改变预测？上述方程已经给出 $\mathcal N(X)=\{0\}\subset\mathbb R^2$。列空间中的元素是三维预测，零空间中的元素是二维参数变化，二者不能互换。若把第二列也改为 $v_1$，新矩阵 $D$ 满足
\[
 Db=(b_0+b_1)v_1,\qquad
 \mathcal N(D)=\operatorname{span}\{(1,-1)^T\}.
\]
只剩一个可识别的参数和 $b_0+b_1$。沿 $(1,-1)^T$ 移动系数不会改变预测。

\textbf{四个基本子空间。}一般矩阵 $A\in\mathbb R^{m\times n}$ 的列空间 $\mathcal C(A)$ 与左零空间 $\mathcal N(A^T)$ 位于 $\mathbb R^m$；行空间 $\mathcal C(A^T)$ 与零空间 $\mathcal N(A)$ 位于 $\mathbb R^n$。本例逐行解 $X^Tr=0$：
\[
 r_1+r_2+r_3=0,\quad r_2+2r_3=0
 \quad\Longrightarrow\quad r=t(1,-2,1)^T.
\]
因此左零空间维数为 1，行空间则由独立行 $(1,0),(1,1)$ 张成整个 $\mathbb R^2$。观察空间分成二维可拟合部分与一维剩余部分；参数空间中没有不可识别方向。

秩--零化度关系 $\operatorname{rank}(A)+\dim\mathcal N(A)=n$ 把参数维数分成可识别与不可识别两部分。对 $X$ 是 $2+0=2$，对重复列矩阵 $D$ 是 $1+1=2$。由 $r^TAx=(A^Tr)^Tx$ 可见，$A^Tr=0$ 当且仅当 $r$ 垂直于所有列组合；这证明了 $\mathcal C(A)^\perp=\mathcal N(A^T)$。将 $A$ 换为 $A^T$，得到另一对正交补关系。
...
先在本例亲手构造正交基。第一列归一化为 $q_1=v_1/\sqrt3$；从第二列减去沿它的投影：
\[
 q_1^Tv_2=\sqrt3,\qquad
 u_2=v_2-(q_1^Tv_2)q_1=(-1,0,1)^T,
 \quad q_2=u_2/\sqrt2.
\]
两列内积为零且长度为 1。把原列用新基表示，得到
\[
 Q=\begin{pmatrix}1/\sqrt3&-1/\sqrt2\\1/\sqrt3&0\\1/\sqrt3&1/\sqrt2\end{pmatrix},
 \quad R=\begin{pmatrix}\sqrt3&\sqrt3\\0&\sqrt2\end{pmatrix},
 \quad Q^Ty=(5/\sqrt3,1/\sqrt2)^T.
\]
从 $R\hat\beta=Q^Ty$ 的第二行先求 $\hat\beta_1=1/2$，再由第一行求 $\hat\beta_0=7/6$。这就是 Gram--Schmidt 加上三角回代。若第二列重复，减去投影后 $u_2=0$，无法归一化；此时它没有提供新的方向，应转到较低维的基或使用伪逆。
...
\]
右奇异向量是参数方向，左奇异向量是输出方向；$Xv_i=\sigma_i u_i$。零奇异值给出不可辨识方向；相对于最大奇异值很小的奇异值，对应相对误差可能被显著放大的方向。伪逆
\[
 X^+=V\Sigma^+U^T
\]
...
\begin{note}
列缩放可能显著改善数值条件，却改变系数单位；ridge 把 $X^TX$ 改为 $X^TX+\lambda I$，同时改变估计目标；删因子改变模型空间。三者都可能让数字更稳定，但分别改变计算坐标、估计目标和模型空间，解释结果时需要区分。
\end{note}
