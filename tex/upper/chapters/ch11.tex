...
反复抛硬币得到的累计得分、一天内到达的订单数和连续变化的价格，都可以用随时间变化的随机变量描述。除了单个时点的分布，我们还关心不同时点怎样联系，以及已知过去后能预测多少。本章依次学习 Markov 链、鞅、Poisson 过程与 Brownian 运动。

先从有限时点与离散状态理解过程，再研究连续时间的路径。命中概率、等待时间和条件期望将成为比较这些模型的具体问题。

\section{分布、样本路径与可用信息}
...
有限维分布描述若干时点的联合变化，样本路径描述一次实现随时间的形状，滤过描述逐步获得的信息。相同的单期分布可以配上不同的时间依赖，因而产生不同的预测与路径性质。
...
\MFQLead{命中概率与期望时长的求解}

对公平游走，$h_{i+1}-h_i=h_i-h_{i-1}$，因此相邻差为常数 $c$。由 $h_0=0$ 得 $h_i=ci$，由 $h_N=1$ 得 $c=1/N$。期望时长方程则给 $m_{i+1}-2m_i+m_{i-1}=-2$；因为 $i^2$ 的二阶差分为 2，试取 $m_i=-i^2+ai+b$，代入两端边界得到 $b=0,a=N$。于是期望命中时长为 $i(N-i)$。
...
三条变换应能直接推导：独立强度 $\lambda_1,\lambda_2$ 的过程叠加后强度相加；对每次到达以概率 $q$ 独立保留，保留与拒绝流分别是强度 $q\lambda$、$(1-q)\lambda$ 的独立 Poisson 过程；非齐次强度 $\lambda(t)$ 的累计强度为 $\Lambda(t)=\int_0^t\lambda(s)\,ds$，经时间变换可还原单位速率过程。例如等待率为 2 时，等待时间的均值、方差为 $0.5,0.25$；强度 2 与 3 的独立过程叠加得到强度 5；对强度 5 的过程取 $q=0.3$，保留与拒绝过程的强度为 $1.5,3.5$；若 $\lambda(t)=2+t$、$t\le3$，累计强度则为 $10.5$。
...
\noindent\textbf{本章要点。}Markov 性使当前状态概括预测所需的历史，鞅性刻画给定信息后的条件均值关系。Poisson 过程描述随机到达，Brownian 运动描述连续高斯波动。命中时间与路径变差进一步说明，仅知道一个时点的分布仍不足以理解整个过程。