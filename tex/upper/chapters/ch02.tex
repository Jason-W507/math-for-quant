...
对度量空间 $(X,d_X)$、$(Y,d_Y)$，当 $x\to a$ 时映射 $T:X\to Y$ 的极限为 $L$，记作 $\lim_{x\to a}T(x)=L$，是指
\[
\forall\varepsilon>0\;\exists\delta>0\;\forall x:\quad
0<d_X(x,a)<\delta\Longrightarrow d_Y(T(x),L)<\varepsilon.
\]
条件 $0<d_X(x,a)$ 表明函数极限只看 $a$ 附近，不要求 $T(a)$ 已定义或等于 $L$。在度量空间中，这等价于：每个满足 $x_n\to a$ 且 $x_n\ne a$ 的序列都有 $T(x_n)\to L$。

映射 $T$ 在 $a$ 连续，是指 $T(a)$ 已定义且 $\lim_{x\to a}T(x)=T(a)$；等价的序列判据为
\[
