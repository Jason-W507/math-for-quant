...
\begin{example}
收益向量 $x,y\in\mathbb{R}^T$ 可用 $\lVert x-y\rVert_2$ 衡量整体误差，也可用 $\lVert x-y\rVert_\infty$ 控制最坏时点误差。前者小不等于每一项都满足同一个业务容差。
\end{example}
...
映射 $T:X\to Y$ 在 $x$ 连续，是指输入足够接近 $x$ 时，输出在 $Y$ 的度量下任意接近 $T(x)$。在度量空间中，这等价于序列判据：
\[
x_n\to x\quad\Longrightarrow\quad T(x_n)\to T(x).
\]
