...
模拟方法的价值不在于把一个简单积分换成更多随机数，而在于为高维、路径依赖或难以解析的对象提供可控的近似。本章先推导 Monte Carlo 的估计量和平方根误差率，再讨论方差缩减，最后说明 bootstrap 何时能够近似抽样分布、何时会因为时间依赖或模型选择而失效。

读者应把“目标不变”和“方差变小”看成两个独立问题。每一种技巧都要先证明它仍在估计同一个对象，再说明误差预算、随机流和有限样本边界；一条平滑的模拟曲线本身不是证据。

\section{从期望到 Monte Carlo 误差}
...
在运行随机数之前，先写清五件事：目标分布 $P$、被积函数或统计量、样本之间的依赖结构、独立基准、允许的误差。若这些对象没有定义，增加模拟次数只会更稳定地计算错误问题。

\begin{example}
设 $U\sim\operatorname{Unif}(0,1)$，目标为 $\mu=\mathbb E[U^2]$。解析积分立即给出 $\mu=1/3$；这不是 Monte Carlo 程序的一部分，而是核对程序的独立基准。真实项目没有闭式解时，也应先寻找可退化到闭式解的参数、小规模可枚举状态或第二种独立算法。
\end{example}

\MFQLead{Monte Carlo 估计量与 $N^{-1/2}$ 误差率}
...
\begin{proposition}[Monte Carlo 中心极限定理的条件]
若 $Y_i=g(X_i)$ 独立同分布、$\mathbb E|Y_i|^2<\infty$，则
\[
 \sqrt N(\hat\mu_N-\mu)\xrightarrow d\mathcal N(0,\sigma^2).
\]
当 $\sigma^2$ 未知时，可用样本方差 $s_N^2$ 构造
$\hat\mu_N\pm z_{1-\alpha/2}s_N/\sqrt N$；这一步还依赖 $s_N^2\to_p\sigma^2$ 和近似正态。若 $Y$ 重尾导致二阶矩不存在，均值仍可能存在但这个区间和 $N^{-1/2}$ 预算都失去依据，必须改用截尾、稳健估计或非渐近尾部界。
\end{proposition}

\input{tex/figures/generated/upper-ch15-mc-rate.tex}
...
$\sqrt{4096/256}=4$。四个标准误不是“永远正确”的保证，而是在有限方差与近似正态误差下声明的显式核对预算。重尾、稀有事件、强自相关和自适应停止都要求另外推导。
...
\begin{example}[路径离散化也会产生偏差]
设连续时间目标为 $\mu=\mathbb E[g(X_{[0,T]})]$，程序用步长 $\Delta t=T/M$ 近似路径并计算 $\hat\mu_{N,M}$。即使 $N\to\infty$，也只会收敛到离散模型的 $\mu_M$；总误差可以写成
\[
 |\hat\mu_{N,M}-\mu|
 \le |\hat\mu_{N,M}-\mu_M|
   +|\mu_M-\mu|.
\]
第一项是抽样误差，通常随 $N^{-1/2}$ 缩小；第二项是离散化偏差，需要比较 $M,2M,4M$ 或使用已知收敛阶外推。只增加路径数量而固定 $M$，会更精确地得到一个有偏的离散目标。
\end{example}

\MFQLead{伪随机数、seed 与并行子流}
...
\begin{note}
复现不是把所有研究永久绑定到一个 seed。更好的证据是：一个登记 seed 用于精确复跑，多个独立 seeds 用于评估结论对随机源的敏感性，再用解析或枚举的独立基准判断程序是否计算了正确对象。
\end{note}
...
\MFQLead{块 bootstrap 如何保留时间依赖}

对长度为 $n$ 的时间序列，先选块长 $\ell$，把连续片段
$B_i=(X_i,\ldots,X_{i+\ell-1})$ 视为重抽样单位；从这些块中有放回抽取并首尾拼接，截到长度 $n$ 后再计算统计量。块长过短会重新打散自相关，过长则有效块数太少；因此应同时报告 $\ell$ 的选择依据、多个块长的敏感性以及时间外覆盖率。若收益标签重叠，块还必须覆盖完整持有期，否则所谓“依赖感知”仍会切断目标窗口。

\MFQLead{可枚举独立基准：样本 $(1,2,4)$}
...
\[
  ...
  =\frac{14}{27}.
\]
这里星号表示“条件于原样本的 bootstrap 世界”。穷举 27 个重抽样均值和 bootstrap 方差为 $14/27$ 构成精确 独立基准；随机重复只应逼近它。

\MFQLead{从重抽样分布构造不确定性}
...
\MFQLead{Bootstrap 何时失效}

IID bootstrap 的关键前提是观察来自共同分布的独立同分布样本（或来自另一个已有理论保证的等价抽样设计），且目标统计量对经验分布足够规则。交换性只能提供分布对称性的直觉，并不蕴含独立性：令所有 $X_i=Y$ 就得到完全交换却完全相关的反例。以下情形需要额外方法或更弱结论：
...
\end{itemize}
对时间序列收益直接做 IID bootstrap 后得到很窄的 Sharpe 区间，并不证明策略稳定。应先定义独立单位和目标统计量，再比较依赖感知方案、不同 block 规则、时间外样本与模拟覆盖率。证据程序在数据声明为有依赖时报错并停止 IID bootstrap，不悄悄输出一个错误区间。

\begin{example}[一个 bootstrap 区间的选择事件]
若原样本均值为 $\bar X=2$，bootstrap 重抽样均值的 2.5\% 和 97.5\% 分位数分别为 $1.4$ 与 $2.8$，percentile 区间写成 $[1.4,2.8]$；basic 区间则为
\[
 [2-(2.8-2),\;2-(1.4-2)]=[1.2,2.6].
\]
两者不同不是排版选择，而是对偏斜和反射方式的不同假设。若先用全样本选择变量再对最终变量 bootstrap，这两个区间都遗漏了选择步骤；必须把选择重新放进每次重抽样或使用独立确认样本。
\end{example}
...
一份独立核对至少覆盖四个层次：
\begin{enumerate}[nosep,leftmargin=*]
  \item 用解析 $1/3$ 与 $4/45$ 核对样本均值及标准误；
  \item 用两个样本量和一千次独立重复核对平方根误差率；
  \item 用解析 $\beta^*=1$、方差 $1/180$ 核对控制变量的均值与方差；
  \item 用有理数穷举 27 个 bootstrap 状态，不从模拟程序生成期望答案。
\end{enumerate}
正常输出只暴露稳定类别，例如 \texttt{error<=4se} 与 \texttt{sample\_vrf>=10}，不把平台相关的随机小数写成契约。两个负例分别把控制变量中心改错、把有依赖数据声明交给 IID bootstrap；它们必须给出稳定诊断。这样固定 seed、理论容差、独立基准和失败协议各自承担不同职责。

\MFQLead{把计算证据写成可解释的研究结论}

面对“你如何验证 Monte Carlo 或 bootstrap”时，可以按以下顺序回答：
\begin{enumerate}[nosep,leftmargin=*]
  \item 定义目标期望或统计量、样本单位和依赖假设；
  \item 给出解析、枚举或退化参数 独立基准；
...
\noindent\textbf{延伸阅读。}金融工程中的 Monte Carlo 参见\cite{glasserman2004montecarlo}，bootstrap 的标准来源参见\cite{efron1993bootstrap}。
...
为一个自己熟悉的模拟任务提交一页审计卡，而不是只提交 notebook。审计卡至少包含：目标 $\mu$ 的数学定义；抽样分布与独立单位；一个解析、枚举或退化参数 独立基准；抽样、离散化与数值误差预算；RNG 与子流协议；方差缩减为何保持目标不变；一个会稳定失败的错误配置；结论的适用边界。

本章 $U^2$ 实验的最小版本可以写成：目标为 $\mathbb E[U^2]=1/3$；解析方差为 $4/45$；固定 seed 只用于复跑，四标准误用于统计核对；控制变量 $U$ 的已知期望为 $1/2$，均值与 VRF 必须同时通过；把中心改成 $0.6$ 应被拒绝；bootstrap 的 27 状态枚举只验证 IID 条件世界，不外推到相关收益。若审计卡无法说明其中任一项，程序输出就还不足以支撑研究结论。

\noindent\textbf{本章要点。}Monte Carlo 的 $N^{-1/2}$ 误差率来自方差推导；方差缩减必须同时保持目标期望；bootstrap 近似的是条件抽样分布，不能脱离依赖和正则性假设。任何模拟结论都应附带解析或枚举基准、误差预算、随机流协议、方差缩减有效性与失败边界。
