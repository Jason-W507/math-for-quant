...
\subsection*{数值编程}
\begin{enumerate}[nosep,leftmargin=*]
  \item 计算正文的三点最小二乘例子，分别用正规方程、QR、\texttt{lstsq} 与伪逆求解；比较残差正交和重构误差。
...
\subsection*{子空间与尺度}
求三点设计矩阵 $X$ 的列空间、零空间及左零空间的一组基，并判断 $(1,2,2)^T$ 是否属于列空间。比较 $10^{-12}I$ 与 $\operatorname{diag}(10^{12},1)$ 的相对条件数。
