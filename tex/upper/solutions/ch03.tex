...
\begin{enumerate}[leftmargin=*]
  ...
  \item 三条定理都先要求所涉函数可测。Fatou 对非负可测函数给 $\int\liminf f_n\leq\liminf\int f_n$；MCT 要求非负可测函数单调递增并给积分等式；DCT 要求 $f_n,f$ 可测、a.e. 收敛以及一个对全部 $n$ 共用的 $g\in L^1$。若 $g$ 不可积，它不能给尾部质量一个有限统一上界，尖峰反例正会穿过该漏洞。
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item 先设 $f_n,f,g$ 可测，$g$ 可积，且 $|f_n|\leq g$、$f_n\to f$ a.e.；于是 $g\pm f_n\geq0$、$|f|\leq g$。Fatou 给
  \[
...
\section*{数值编程}
\begin{enumerate}[leftmargin=*]
  \item 等差和给 $1/2-2^{-m-1}$，分别为 $0.375,0.46875,0.498046875$。
  \item 面积恒为 1，任意固定正点最终取零；零点也取零，所以逐点趋零但积分不趋零。
  \item 指数 1 时为 $1+\log n$，极限无穷；指数 2 时为 $2-1/n$，可用可积的 $X$ 支配截断变量。
\end{enumerate}

\section*{研究判断}

\begin{enumerate}[leftmargin=*]
  \item 两条路线都先证明 $L_n$ 与 $L$ 可测。路线一再证明 $0\leq L_n\uparrow L$ 并用 MCT；路线二是找到共同的可积 $G$，证明 $|L_n|\leq G$ 且 a.e. 收敛，再用 DCT。只有非负性时，Fatou 通常只能给单侧不等式。还要声明期望是否允许为无穷。
  ...
  \item 审计应包含：明确测度空间和可测性；证明绝对可积或给出尾部界；列出 MCT/DCT 的逐条假设；构造共同支配函数并验证其积分；保留解析解或独立计算；使用自适应网格或区间界检查移动峰值；对截断域给出尾部误差；最后报告有限离散证据不能覆盖的一般性限制。
\end{enumerate}

\section*{基础衔接：独立计算}
$\mathbb EX=2\int_1^\infty x^{-2}dx=2$；$\mathbb EX^2=2\int_1^\infty x^{-1}dx=\infty$。$X^2$ 不是可积支配函数；有限一阶矩不推出有限二阶矩。
