...
\end{center}
正常路径必须同时给出 timeline、split、multiplicity、performance 和 friction 五类通过证据。随后复制 oracle：先把第一行 \path{available_at} 改到 \path{decision_at} 之后，确认时间门禁失败；再恢复时间并把 \path{selected_raw_p_value} 改为 $0.003$，确认多重检验门禁失败。恢复完成的标准不是“改回能跑”，而是能解释两个负例分别破坏了哪个研究假设，以及为什么增加样本或更换随机 seed 都无法修复。
