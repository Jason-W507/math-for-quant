...
\begin{enumerate}[leftmargin=*]
  ...
  \item 一百万次随机搜索没有发现反例，只能说明抽到的有限样本通过，不能覆盖连续定义域。更强证据包包含：一般证明，明确每条假设；独立 oracle 验证代表性算例；只删除非负性并保留归一化的 $w'=(1.5,-0.5,0)$，产生 $0.035>0.03$；以及会拒绝伪反例和错维输入的程序门禁。证明负责普遍性，oracle 负责实现一致性，失败见证负责边界。
\end{enumerate}
