...
\section*{数值编程}
\begin{enumerate}[leftmargin=*]
  \item 手算二次型值为 $-1$，梯度为 $(0,-0.5)^T$；链式梯度为 $(4,-6.5)^T$。固定 $h=10^{-5}$ 时最大差异为 $2.62\times10^{-11}$，最大差异小于 $10^{-9}$。对 $|t|$，左差商为 $-1$、右差商为 $1$，因此零点不可微。继续扫描 $10^{-2}$ 至 $10^{-8}$：较大 $h$ 由截断误差主导，极小 $h$ 由消去误差主导；报告机器与数据类型，因为最低点并非普适常数。
  ...
  \item 取非对称 $A$，正确梯度为 $((A+A^T)/2)w-\lambda\mu$，错误实现为 $Aw-\lambda\mu$。在正文固定 $A$ 与 $w=(0.5,-1)^T$ 上，两者不同；中心差分匹配正确的对称化公式。若只用对称矩阵，错误实现会被测试掩盖，因此测试数据必须激活被检查的分支。
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item 先写损失是 $\sum_i\ell_i$ 还是 $n^{-1}\sum_i\ell_i$，手算两者梯度相差 $n$。然后用单样本和重复样本测试观察缩放是否线性；检查广播是否把 $(n,1)$ 与 $(n,)$ 扩成 $(n,n)$；最后核对研究定义究竟要求总成本还是平均风险。单位分析可进一步发现收益、基点和金额混用。
  \item 第一层用有限差分/自动微分确认 $\nabla L=\Sigma w-\lambda\mu$，这是导数正确性。第二层检查 $\Sigma$ 的特征值、条件数和扰动放大，使用分解或正则化处理病态性。第三层审查期望收益估计误差、样本外稳定性、预算/杠杆/换手约束和交易成本；闭式解极端即使数学和数值都正确，也可能没有研究可用性。
\end{enumerate}
