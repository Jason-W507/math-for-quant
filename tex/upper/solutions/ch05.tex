...
\section*{数值编程}
\begin{enumerate}[leftmargin=*]
  \item 手算二次型值为 $-1$，梯度为 $(0,-0.5)^T$；链式梯度为 $(4,-6.5)^T$。固定 $h=10^{-5}$ 时最大差异为 $2.62\times10^{-11}$，最大差异小于 $10^{-9}$。对 $|t|$，左差商为 $-1$、右差商为 $1$，因此零点不可微。固定扫描中，$h=10^{-12}$ 时误差超过 $10^{-6}$，明确展示消去误差回升；最低点仍依赖机器和数据类型。
