...
class EditorialReleaseContractTests(unittest.TestCase):
    ...
    def test_v03_metadata_and_lower_generated_navigation_are_release_ready(self) -> None:
        ...
        self.assertIn("name: ${{ github.ref_name }} — 视觉与笔面试训练增强版", release_workflow)
        ...
        self.assertIn("python tools/build_books.py --volume all", release_workflow)
        self.assertIn("Verify tag matches VERSION", release_workflow)
        self.assertNotIn("tools/build_release.py", release_workflow)
        self.assertNotIn("math-for-quant-notebooks.zip", release_workflow)
        self.assertNotIn("release-manifest.json", release_workflow)
