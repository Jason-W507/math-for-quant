...
class ChaptersTests(LearningUnitCase):
    ...
    def test_chapter_seven_rejects_negative_probability_mass(self) -> None:
        result = self.run_oracle_fixture(
            "ch07-negative-mass.json",
            "evidence/ch07/oracle.json",
            "notebooks/upper/ch07_probability_distributions.py",
            lambda document: document.update(
                {
                    "finite_probabilities": [-0.25, 0.75, 0.5],
                    "expected_mean": 1.25,
    ...
    def test_chapter_fourteen_notebook_reproduces_numerical_oracles(self) -> None:
        ...
        self.assertEqual(
            ...
            "leastsq=(condition~2.449e+06,normal-condition>=condition^1.9,"
            "normal-error<=1.0e-02,stable-error<=5.0e-04,"
    ...
    def test_chapter_fifteen_notebook_reproduces_simulation_oracles(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "tools" / "build_notebook.py"),
                "--source",
                "notebooks/upper/ch15_monte_carlo.py",
                "--output",
                "build/notebooks/upper/ch15_monte_carlo.ipynb",
                "--execute",
            ],
            cwd=ROOT,
            text=True,
        ...
        self.assertEqual(
            result.stdout,
            "notebook=build/notebooks/upper/ch15_monte_carlo.ipynb\n"
            "roundtrip=passed cells=2\n"
            "execution=passed oracle=passed "
    ...
    def test_chapter_fifteen_rejects_iid_bootstrap_for_dependent_data(self) -> None:
        result = self.run_oracle_fixture(
            "ch15-dependent.json",
            "evidence/ch15/oracle.json",
            "notebooks/upper/ch15_monte_carlo.py",
            lambda document: document.update({"bootstrap_observations_iid": False}),
        )

        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(
            result.stderr,
            "iid bootstrap invalid: observations are dependent; use a dependence-aware resampling scheme\n",
        )

    def test_chapter_fifteen_rejects_an_unreasonably_tight_mc_budget(self) -> None:
        result = self.run_oracle_fixture(
            "ch15-tight-budget.json",
            "evidence/ch15/oracle.json",
            "notebooks/upper/ch15_monte_carlo.py",
            lambda document: document.update({"confidence_multiplier": 1.0}),
        )
    ...
    def test_chapter_fifteen_rejects_a_miscentered_control_variate(self) -> None:
        result = self.run_oracle_fixture(
            "ch15-control-mean.json",
            "evidence/ch15/oracle.json",
            "notebooks/upper/ch15_monte_carlo.py",
            lambda document: document.update({"control_known_mean": 0.6}),
        )
