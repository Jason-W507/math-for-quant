"""Render curriculum registries and reader-facing exercise attribution."""
...
def render_course_map(manifest: dict[str, object]) -> str:
    ...
    for bridge in manifest["reader_bridges"]:
        ...
        bridge_sections.append(
            ...
            f"采用{tex_text('、'.join(route_titles))}的读者，可在第 {before} 章前阅读。"
            f"需要完整证明时回看{review}。\n"
        )
    ...
    return (
        ...
        + "\n\n".join(route_sections)
        + "\n\\begin{note}\n"
        "概率空间、期望交换、条件化与 $L^p$ 论证的完整基础位于第 3、4、8 章。"
        "应用主线可按需要回读，理论增强线依次学习这些章节。\n"
        "\\end{note}\n\n"
        "\\section{应用主线桥接}\n\n"
...
def render_interview_coverage() -> str:
    """Render original attribution without maintenance coverage statistics."""
    ledger = json.loads(INTERVIEW_LEDGER_SOURCE.read_text(encoding="utf-8"))
    manifest = json.loads(MANIFEST_SOURCE.read_text(encoding="utf-8"))
    titles = {str(unit["id"]): str(unit["title"]) for unit in manifest["units"]}
    for unit in manifest["units"]:
        source = unit.get("evidence", {}).get("core_derivation")
        if unit.get("volume") == "lower" and source:
            titles.setdefault(f"lower.{Path(source).stem}", str(unit["title"]))
    entries = [item for item in ledger["problems"] if item.get("home_unit")]
    source = ledger["source"]
    lines = [
        "% Generated by tools/build_course_assets.py; edit curriculum/interview-problem-ledger.json instead.",
        f"补充习题参考 {tex_text(source['author'])} 的《{tex_text(source['title'])}》"
        f"（{tex_text(source['edition'])}）。下表列出原题名及本书位置。"
        "题目按本书的符号与教学顺序重新表述，解答独立编写；题源用于进一步阅读与对照。",
        r"\begin{longtable}{p{0.14\textwidth}p{0.38\textwidth}p{0.34\textwidth}}",
        "编号 & 原书题名 & 本书位置 " + r"\\ \hline",
    ]
    for item in entries:
        home = str(item["home_unit"])
        location = prerequisite_name(home, titles)
        lines.append(f"{item['id']} & {tex_text(str(item['original_title']))} & {location} " + r"\\")
