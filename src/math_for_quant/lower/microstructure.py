...
def poisson_mle(interarrivals: list[float]) -> tuple[float, float]:
    if not interarrivals or any(value <= 0.0 for value in interarrivals):
...
class OrderBook:
    ...
    def add(self, order_id: str, side: str, price: float, quantity: int) -> None:
        if side not in {"buy", "sell"} or price <= 0.0 or quantity <= 0:
...
- 执行：TWAP 为 ({int(observed['twap_1'])}, {int(observed['twap_2'])}, {int(observed['twap_3'])})，冲击成本 {observed['twap_cost']:.6f}；库存惩罚后的最优序列为 ({int(observed['optimal_1'])}, {int(observed['optimal_2'])}, {int(observed['optimal_3'])})，总目标 {observed['optimal_cost']:.6f}。
- 失败边界：日内季节性、Hawkes 不稳定、错误撤单、零冲击、无限流动性与未来价格可见分别拒绝。
...
def main(oracle_path: Path = Path("evidence/lower-ch06/oracle.json")) -> int:
    ...
    twap_cost = impact * sum(quantity * quantity for quantity in twap)
    optimal, optimal_cost = optimal_execution(
        inventory, steps, impact, float(oracle["inventory_risk"]),
    ...
    observed: dict[str, float | int] = {
        ...
        "twap_1": twap[0], "twap_2": twap[1], "twap_3": twap[2], "twap_cost": twap_cost,
        "optimal_1": optimal[0], "optimal_2": optimal[1], "optimal_3": optimal[2], "optimal_cost": optimal_cost,
    ...
    print(
        ...
        f"{twap_cost:.6f},{optimal_cost:.6f}) failures=({','.join(str(value) for value in failures)})"
    )
    return 0
