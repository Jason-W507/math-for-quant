...
from math_for_quant.lower.derivatives import black_scholes_call, call_delta
...
def vectorized_delta_hedges(
    *,
    ...
    delta = np.full(paths, call_delta(spot, strike, rate, sigma, maturity))
    ...
    for index in range(1, steps + 1):
        ...
        if index < steps:
            ...
            next_delta = np.asarray(
                [call_delta(value, strike, rate, sigma, remaining) for value in current]
            )
