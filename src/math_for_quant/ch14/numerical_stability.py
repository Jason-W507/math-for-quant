...
def main(oracle_path: Path = Path("evidence/ch14/oracle.json")) -> int:
    ...
    print(
        ...
        f"leastsq=(condition~{float(expected['least_squares_condition']):.3e},"
        "normal-condition>=condition^1.9,"
        f"normal-error<={float(limits['maximum_normal_equation_relative_error']):.1e},"
        f"stable-error<={float(limits['maximum_stable_relative_error']):.1e},"
        f"stable-residual<={float(limits['maximum_stable_residual']):.1e},"
        f"rank={least_rank}) "
